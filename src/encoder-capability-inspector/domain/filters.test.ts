...
describe("experimental filter", () => {
  const production = videoResultFixture({ id: "production" });
  const experimental = videoResultFixture({
    id: "experimental",
    experimental: true,
    experimentalReasons: ["bit-depth-10"],
  });
  const audio = audioResultFixture({ id: "audio" });

  it("keeps everything when unset", () => {
    for (const result of [production, experimental, audio]) {
      expect(matchesFilters(result, EMPTY_RESULT_FILTERS)).toBe(true);
    }
  });

  it("narrows to the experimental configurations", () => {
    const filters = withFilters({ experimental: "only" });
    expect(matchesFilters(experimental, filters)).toBe(true);
    expect(matchesFilters(production, filters)).toBe(false);
  });

  it("drops the experimental configurations", () => {
    const filters = withFilters({ experimental: "exclude" });
    expect(matchesFilters(experimental, filters)).toBe(false);
    expect(matchesFilters(production, filters)).toBe(true);
  });

  it("treats audio as a production configuration", () => {
    // 音声候補に experimental の概念は無い。除外しても残り、絞り込むと消える。
    expect(
      matchesFilters(audio, withFilters({ experimental: "exclude" })),
    ).toBe(true);
    expect(matchesFilters(audio, withFilters({ experimental: "only" }))).toBe(
      false,
    );
  });

  it("counts as a filter for the empty check", () => {
    expect(isFiltersEmpty(withFilters({ experimental: "only" }))).toBe(false);
  });
});
