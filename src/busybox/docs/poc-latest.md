...
未実装・実行待ち・仕様待ちを含む今後の作業順は[残存PoC実装計画](./remaining-poc-implementation-plan.md)を正とする。
...
| S-800 | POC-032 | Text Fragment assembly | `beforematch`で閉じた対象語を出現 |
...
| S-810 | 固定asset検証 | `fixtures/s810/assets.test.ts` / `stages/S-810.functions.test.ts` | pack・manifest・4目標比率・5%許容判定 |
...
| S-510 | POC-009 | sandbox iframeのnative image drag・別window・実第二origin |
| S-430-B02 | POC-033 | Safari/WebKit Audio Session対応環境 |
| S-350-B07 | POC-034監視枠 | `audioTracks`を公開するbrowser |
...
| S-770 | POC-027 | PoCはMockFedCM、製品化時は公式FedCM provider・公開RP |
| S-780 | POC-028 | local Viteの`Link`応答header確認済み・foreground Payment Handler対応ChromeでUI lifecycleを確認 |
...
- POC-007: S-430の旧Audio Session試作。現行の承認済みB02はPOC-033として再実装し、旧番号は再利用しない。B01のMedia Session pause actionはPOC-031で扱う。
