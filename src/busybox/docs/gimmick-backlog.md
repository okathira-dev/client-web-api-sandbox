...
| G-077 | 三つの財布 | 架空payment handlerのhandler windowで承認、意図的拒否、同一handler再試行を別々に成立させる。handler選択や開始経路は固定しない | browser所有のhandler候補、trusted PaymentRequestEvent、handler window、complete / retryというpayment lifecycleを3箱へ分ける。trusted eventは証跡として扱い単独の箱にしない | Labs + Chromium系 + HTTPS + Service Worker | H-003, H-004, H-019, H-023, H-025, H-050 | 採用・S-780 PoC確認済み、製品stage未実装。各条件成立時に箱だけを開き、結果label、完了message、固定flagを表示しない |
