...
| G-068 | 断片をたどる文書 | 同一page内のText Fragment linkを順に辿り、browserが示した複数の一節からhintを集めて最終回答を作る | URLそのものを長文内の移動手段にし、scriptから読めないUA highlightを手掛かりとして使う | Labs + browser依存 | H-054 | S-690製品実装済み・人手確認待ち。4 jumpをscriptで数えず、`busybox{text_fragments_leave_trails}`の完全一致だけで開く |
