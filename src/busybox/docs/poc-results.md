...
> 初回実施日: 2026-08-01、POC-006 / POC-011 / POC-013 / POC-021 / POC-022 fixture更新: 2026-08-02。実装前PoCの正本は[実装PoCマスタープラン](./implementation-poc-master-plan.md)。ここでは実施済みの最小検証と、その時点で実資源がなく完遂できない検証を区別して記録する。
...
| 008 S-480 | FAIL | User Preferences override APIが現在のChromeにない。 | 対応browserを確認できるまで実装を開始しない。CSS模倣はしない。 |
| 009 S-510 | PARTIAL | `DataTransfer` と `DragEvent` は存在。 | installed PWA、別origin iframe、window境界をまたぐ実dragは未実施。 |
...
| 020 S-700-B03 | PARTIAL | Presentation APIは存在。 | 外部画面への`PresentationRequest.start()`とreceiver readyは未実施。 |
...
| 027 S-770 | PARTIAL | IdentityCredential/FedCM入口は存在。 | 公式providerの現行監査、RP client登録、実accountのFedCM専用結果が未実施。 |
| 028 S-780 | PARTIAL | PaymentRequest と Payment Handler関連prototypeは存在。 | public HTTPS、manifest、handler SW、PaymentRequestEvent、approve/reject/retryは未実施。 |
| 029 S-790 | FAIL | Local Font Access APIが現在のChromeにない。 | desktop Chromium対応環境＋専用OTFのOS user installで再PoCする。font列挙はしない。 |
...
- D-137で、現環境にAPIまたは中心経路があり、製品実装によって残る不確実性を実stage上で確認できる箱は一括実装へ進めることにした。対象とWaveは[現環境確認分の一括実装計画](./current-environment-implementation-plan.md)を正とする。
...
2. public HTTPS / installed PWA: S-510、S-740、S-780。S-060-B02のbackend停止経路はlocalhost PoCで合格済み。
