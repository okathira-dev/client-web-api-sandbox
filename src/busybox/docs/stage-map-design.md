...
- 中央にBusybox本体を置き、仮の5系統を大branchとして放射する。
- stage nodeは既存と同じsemanticなbutton / linkと箱DOMを使う。進捗のリボン、閉箱、開箱を地図でも維持する。
...
- screen readerにはstage名、進捗、主branchを読み上げる。related / clue関係は冗長にならない短い補助説明として関連付ける。
...
実装順1・2と一覧の主要導線切替を完了した。`StageSpec.map`を全catalogue項目の必須情報とし、未指定の既存stageにもカテゴリとIDから安定したbranch / orderを導出する。表示は2700×1900の決定的なDOM map、背面SVG edge、70〜130%の段階zoom、中央復帰、外縁marker slotから構成する。stage nodeは従来の`StageCard`をそのまま使うため、routing、永続進捗、keyboard focus、accessible nameは地図の描画方式から独立している。
