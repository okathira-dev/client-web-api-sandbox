...
現在はアプリシェル、日英表示、ステージ台帳、基本ナビゲーション、IndexedDB進捗スキーマ、進捗の書き出し・初期化、共通ステージランタイム、同形・同寸法で入場ごとに再挑戦できる全26問題箱、全21ステージ、Busybox scope限定のPWA、任意のGoogle Driveバックアップを備える。
