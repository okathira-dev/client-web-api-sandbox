 ...
 */
export default function S760Stage(props: StageComponentProps) {
  const fullProblem = props.problem("S-760-B01");
  const emptyProblem = props.problem("S-760-B02");
  const [status, setStatus] = useState(() =>
    stageText(props.locale, s760Locale.idle),
  );
  ...
  const select = async (emptyMode: boolean) => {
    ...
    if (!contacts?.select) {
      setStatus(stageText(props.locale, s760Locale.unavailable));
      return;
    }
    try {
      ...
      if (emptyMode && hasNoSharedS760Properties(contact)) {
        emptyProblem.solve(["contact:selected-one", "contact:shared-none"]);
        setStatus(stageText(props.locale, s760Locale.emptySuccess));
      } else if (!emptyMode && matchesS760Card(contact)) {
        fullProblem.solve([
          "contact:five-properties",
          "contact:fictional-card",
        ]);
        setStatus(stageText(props.locale, s760Locale.fullSuccess));
      } else {
        setStatus(stageText(props.locale, s760Locale.mismatch));
      }
    } catch {
      setStatus(stageText(props.locale, s760Locale.cancelled));
    }
  };
  ...
  return (
    <div className="puzzle puzzle--centered">
      <div className="problem-row">
        <ProblemGiftBox problem={fullProblem} locale={props.locale} />
        <ProblemGiftBox problem={emptyProblem} locale={props.locale} />
      </div>
      <article className="s760-card">
        <img
          src={image}
          alt={stageText(props.locale, s760Locale.iconAlt)}
          width={512}
          height={512}
        />
        <dl>
          <dt>{stageText(props.locale, s760Locale.nameLabel)}</dt>
          <dd>Busybox Courier</dd>
          <dt>{stageText(props.locale, s760Locale.emailLabel)}</dt>
          <dd>courier@busybox.invalid</dd>
          <dt>{stageText(props.locale, s760Locale.telLabel)}</dt>
          <dd>+81 3-0000-0000</dd>
          <dt>{stageText(props.locale, s760Locale.addressLabel)}</dt>
          <dd>1-1-1 Busybox, Tokyo 100-0001, Japan</dd>
          <dt>{stageText(props.locale, s760Locale.iconLabel)}</dt>
          <dd>
            <a href={image} download="busybox-courier.svg">
              {stageText(props.locale, s760Locale.saveIcon)}
            </a>
          </dd>
        </dl>
      </article>
      <p>{stageText(props.locale, s760Locale.idle)}</p>
      <div className="stage-action-row">
        ...
        >
          {stageText(props.locale, s760Locale.full)}
        </button>
        <button
          type="button"
          className="stage-action"
        ...
        >
          {stageText(props.locale, s760Locale.empty)}
        </button>
      </div>
