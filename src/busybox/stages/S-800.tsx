 ...
 */
export default function S800Stage(props: StageComponentProps) {
  const encodedProblem = props.problem("S-800-B01");
  const wordProblem = props.problem("S-800-B02");
  ...
  useEffect(() => {
    ...
    const handleEncoded = () => {
      encodedProblem.solve(["text-fragment:encoded-container-beforematch"]);
      setStatus(stageText(props.locale, s800Locale.revealed));
    };
    const handleWord = () => {
      wordProblem.solve(["text-fragment:word-container-beforematch"]);
      setStatus(stageText(props.locale, s800Locale.revealed));
    };
  ...
  return (
    <div className="puzzle s800-stage">
      <div className="problem-row">
        <ProblemGiftBox problem={encodedProblem} locale={props.locale} />
        <ProblemGiftBox problem={wordProblem} locale={props.locale} />
      </div>
      <p>{stageText(props.locale, s800Locale.intro)}</p>
      <div className="s800-prompts">
        <section>
          <h2>{stageText(props.locale, s800Locale.fragmentLabel)}</h2>
          <code>{encodedFragment}</code>
          <p>{stageText(props.locale, s800Locale.fragmentHelp)}</p>
        </section>
        <section>
          <h2>{stageText(props.locale, s800Locale.wordLabel)}</h2>
          <WordCanvas word="ember" />
          <p>{stageText(props.locale, s800Locale.wordHelp)}</p>
        </section>
      </div>
