...
import manifest from "../fixtures/s790/assets/generation-manifest.json";
 ...
 */
export default function S790Stage(props: StageComponentProps) {
  const problem = props.problem("S-790-B01");
  const glyphRef = useRef<HTMLSpanElement>(null);
  const loadedRef = useRef<{ face: FontFace; url: string } | undefined>(
    undefined,
  );
  const [status, setStatus] = useState(() =>
    stageText(props.locale, s790Locale.idle),
  );
  ...
  const scan = async () => {
    ...
    if (!queryLocalFonts) {
      setStatus(stageText(props.locale, s790Locale.unavailable));
      return;
    }
    clear();
    setStatus(stageText(props.locale, s790Locale.scanning));
    try {
      const fonts = await queryLocalFonts({
        postscriptNames: [manifest.postscriptName],
      });
      if (
        fonts.length !== 1 ||
        fonts[0]?.postscriptName !== manifest.postscriptName
      ) {
        setStatus(stageText(props.locale, s790Locale.missing));
        return;
      }
      ...
      if (digest !== manifest.sha256) {
        setStatus(stageText(props.locale, s790Locale.mismatch));
        return;
      }
      ...
      problem.solve([
        "local-font:limited-query",
        "local-font:exact-digest",
        "local-font:glyph-loaded",
      ]);
      setStatus(stageText(props.locale, s790Locale.success));
    } catch {
      clear();
      setStatus(stageText(props.locale, s790Locale.cancelled));
    }
  };
  ...
  return (
    <div className="puzzle puzzle--centered">
      ...
      <p>{stageText(props.locale, s790Locale.instruction)}</p>
      <a className="stage-action" href={fontAsset} download="busybox-key.ttf">
        {stageText(props.locale, s790Locale.download)}
      </a>
      <figure className="s790-glyph">
        <span ref={glyphRef}></span>
        <figcaption>
          {stageText(props.locale, s790Locale.glyphLabel)}
        </figcaption>
      </figure>
      <div className="stage-action-row">
        <button
          ...
          onClick={() => void scan()}
        >
          {stageText(props.locale, s790Locale.scan)}
        </button>
        <button
          type="button"
          className="stage-action"
          onClick={() => {
            clear();
            setStatus(stageText(props.locale, s790Locale.cleared));
          }}
        >
          {stageText(props.locale, s790Locale.clear)}
        </button>
      </div>
