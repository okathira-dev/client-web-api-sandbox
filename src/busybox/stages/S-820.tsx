...
const targetDefinitions = [
  { id: "S-820-B01", x: 800, y: -600 },
  { id: "S-820-B02", x: -3000, y: 4000 },
  { id: "S-820-B03", x: 6000, y: 8000 },
] as const satisfies readonly { id: ProblemBoxId; x: number; y: number }[];
 ...
 */
export default function S820Stage(props: StageComponentProps) {
  const planeRef = useRef<HTMLDivElement>(null);
  const [position, setPosition] = useState({ x: 0, y: 0 });
  const [locked, setLocked] = useState(false);
  const problems = targetDefinitions.map((target) => ({
    ...target,
    problem: props.problem(target.id),
  }));
  ...
  return (
    <div className="puzzle puzzle--centered s820-stage">
      ...
      <p>{stageText(props.locale, s820Locale.intro)}</p>
      <button
        type="button"
        className="stage-action"
        onClick={() => planeRef.current?.requestPointerLock()}
      >
        {stageText(props.locale, s820Locale.begin)}
      </button>
      <section
        ref={planeRef}
        ...
        aria-label={stageText(
          props.locale,
          locked ? s820Locale.locked : s820Locale.unlocked,
        )}
      >
        <span className="s820-reticle" aria-hidden={!nearby}>
          {nearby ? (
            <ProblemGiftBox
              problem={nearby.problem}
              locale={props.locale}
              onClick={(event) => {
                if (
                  event.isTrusted &&
                  ...
                  nearby.problem.solve([`pointer-lock:target:${nearby.id}`]);
                }
              }}
            />
          ) : (
      ...
      <output className="s820-position" aria-live="polite">
        {stageText(props.locale, s820Locale.position)}: (
        ...
        {problems.map((target) => {
          ...
          return (
            <li key={target.id}>
              ...
              {stageText(props.locale, s820Locale.target)}{" "}
      ...
      <p className="interaction-status">
        {stageText(props.locale, nearby ? s820Locale.nearby : s820Locale.noBox)}
      </p>
    </div>
  );
}
