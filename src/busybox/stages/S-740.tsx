 ...
 */
export default function S740Stage(props: StageComponentProps) {
  const problem = props.problem("S-740-B01");
  const solveProblem = problem.solve;
  const [state, setState] = useState<S740State>(initialS740State);
  const [registered, setRegistered] = useState(false);
  const [status, setStatus] = useState(() =>
    stageText(props.locale, s740Locale.idle),
  );
  ...
  const inspect = useCallback(async () => {
    try {
      ...
      if (next.phase === 2 && next.clientlessEvents >= 2) {
        ...
        if (cached) {
          solveProblem(["periodicsync:clientless-events:2", "cache:bloom.svg"]);
          setStatus(stageText(props.locale, s740Locale.bloom));
          return;
        }
      }
      if (next.phase === 1 && !next.pendingCare)
        setStatus(stageText(props.locale, s740Locale.sprout));
      else if (next.pendingCare === "water")
        setStatus(stageText(props.locale, s740Locale.waterLeft));
      else if (next.pendingCare === "light")
        setStatus(stageText(props.locale, s740Locale.lightLeft));
      else setStatus(stageText(props.locale, s740Locale.waiting));
    } catch {
      setStatus(stageText(props.locale, s740Locale.failed));
    }
  ...
  const register = async () => {
    try {
      ...
      if (!registration.periodicSync) {
        setStatus(stageText(props.locale, s740Locale.unsupported));
        return;
      }
      ...
      setRegistered(true);
      setStatus(stageText(props.locale, s740Locale.registered));
    } catch {
      setStatus(stageText(props.locale, s740Locale.failed));
    }
  };
  ...
  const leaveCare = async (care: "water" | "light") => {
    try {
      ...
      setStatus(
        stageText(
          props.locale,
          care === "water" ? s740Locale.waterLeft : s740Locale.lightLeft,
        ),
      );
    } catch {
      setStatus(stageText(props.locale, s740Locale.failed));
  ...
  const reset = async () => {
    try {
      ...
      setRegistered(false);
      setStatus(stageText(props.locale, s740Locale.cleared));
    } catch {
      setStatus(stageText(props.locale, s740Locale.failed));
    }
  };
  ...
  return (
    <div className="puzzle puzzle--centered">
      ...
      <img
        ...
        src={imageUrl}
        alt={stageText(props.locale, s740Locale.imageAlt)}
        width={480}
        height={320}
      />
      <div className="stage-action-row">
        ...
        >
          {stageText(props.locale, s740Locale.register)}
        </button>
        <button
          type="button"
          className="stage-action"
        ...
        >
          {stageText(props.locale, s740Locale.water)}
        </button>
        <button
          type="button"
          className="stage-action"
        ...
        >
          {stageText(props.locale, s740Locale.light)}
        </button>
        <button
          type="button"
          className="stage-action"
        ...
        >
          {stageText(props.locale, s740Locale.refresh)}
        </button>
        <button
          type="button"
          className="stage-action"
          onClick={() => void reset()}
        >
          {stageText(props.locale, s740Locale.reset)}
        </button>
      </div>
