...
import { GiftBox, ProblemGiftBox } from "../ui/GiftBox";
...
function goalLocaleKey(problemId: "S-920-B01" | "S-920-B02" | "S-920-B03") {
  return problemId.slice(-3) as "B01" | "B02" | "B03";
}
 ...
 */
export default function S920Stage(props: StageComponentProps) {
  ...
  const solveGoal = (node: S920MazeNode) => (event: ReactMouseEvent) => {
    if (!node.problemId || !event.isTrusted) return;
    if (!nodeRefs.current.get(node.id)?.matches(":popover-open")) return;
    props
      .problem(node.problemId)
      .solve([`popover-maze:${node.id}:trusted-goal-click`]);
  };
  ...
  const renderNode = (nodeId: string): React.ReactNode => {
    ...
    const goalProblem = node.problemId
      ? props.problem(node.problemId)
      : undefined;
    const isDeadEnd = !goalProblem && children.length === 0;
    const goalLabel = node.problemId
      ? stageText(props.locale, s920Locale[goalLocaleKey(node.problemId)])
      : undefined;
        ...
        <div className="s920-popover__room">
          ...
          {isDeadEnd ? (
            <span className="s920-popover__dead-end">
              {stageText(props.locale, s920Locale.deadEnd)}
            </span>
          ) : null}
          {children.length > 0 ? (
                ...
                return (
                  <button
                    ...
                    aria-label={stageText(
                      props.locale,
                      s920Locale[localeKeyByDirection[childDirection]],
                    )}
                  >
  ...
  const renderShadowRoute = (goal: (typeof s920GoalNodes)[number]) => {
    ...
    const problem = props.problem(goal.problemId);
              ...
              <div className="s920-popover__room">
                {nextNode ? (
                  ...
                  <div className="problem-gift s920-shadow-gift">
                    <GiftBox
                      state="closed"
                      color={problem.definition.color}
                      label=""
                      decorative
                    />
  ...
  const frameContent = (
    <div className="s920-frame-content">
      <div className="s920-maze" aria-busy={layoutState === "preparing"}>
        ...
        >
          {stageText(props.locale, s920Locale.start)}
        </button>
  ...
  return (
    <div className="puzzle s920-stage">
      <p>{stageText(props.locale, s920Locale.intro)}</p>
      <div className="s920-frame-shell">
        <span className="s920-frame-shell__label">
          {stageText(props.locale, s920Locale.unavailableArea)}
        </span>
        <iframe
          ref={frameRef}
          className="s920-frame"
          title={stageText(props.locale, s920Locale.frameTitle)}
        ...
        {stageText(
          ...
          layoutState === "ready"
            ...
            : layoutState === "unavailable"
              ? s920Locale.unavailable
              : s920Locale.preparing,
        )}
      </output>
    </div>
  );
