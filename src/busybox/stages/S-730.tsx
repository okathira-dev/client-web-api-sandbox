 ...
 */
export default function S730Stage(props: StageComponentProps) {
  const sessionProblem = props.problem("S-730-B01");
  const selectProblem = props.problem("S-730-B02");
  ...
  const [status, setStatus] = useState(() =>
    stageText(props.locale, s730Locale.idle),
  );
  ...
  const start = async () => {
    ...
    if (!xr || !canvas) {
      setStatus(stageText(props.locale, s730Locale.unsupported));
      return;
    }
    await cleanup();
    setStatus(stageText(props.locale, s730Locale.starting));
    try {
      ...
      const onSelect = (event: XRInputSourceEvent) => {
        ...
        if (new Raycaster(origin, direction).intersectObject(box).length === 0)
          return;
        selectProblem.solve(["xr:input-source", "xr:box-ray-hit"]);
        setStatus(stageText(props.locale, s730Locale.selected));
      };
      ...
      session.addEventListener(
        ...
        () => {
          ...
          dispose();
          setStatus(stageText(props.locale, s730Locale.ended));
        },
        { once: true },
      );
      runtimeRef.current = {
      ...
      renderer.setAnimationLoop((_time, frame) => {
        if (frame && !poseSeen && frame.getViewerPose(referenceSpace)) {
          poseSeen = true;
          sessionProblem.solve(["xr:immersive-session", "xr:viewer-pose"]);
          setStatus(stageText(props.locale, s730Locale.pose));
        }
        renderer.render(scene, camera);
      });
    } catch {
      setStatus(stageText(props.locale, s730Locale.cancelled));
      await cleanup();
    }
  };
  ...
  return (
    <div className="puzzle puzzle--centered">
      <div className="problem-row">
        <ProblemGiftBox problem={sessionProblem} locale={props.locale} />
      ...
      <div className="stage-action-row">
        <button
          ...
          onClick={() => void start()}
        >
          {stageText(props.locale, s730Locale.start)}
        </button>
        <button
          type="button"
          className="stage-action"
        ...
        >
          {stageText(props.locale, s730Locale.end)}
        </button>
      </div>
