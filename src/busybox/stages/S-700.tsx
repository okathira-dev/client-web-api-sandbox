...
import manifest from "../fixtures/s700/assets/generation-manifest.json";
...
type RemoteSlot = (typeof manifest.assets)[number];
const remoteSlots = manifest.assets as [RemoteSlot, ...RemoteSlot[]];
 ...
 */
export default function S700Stage(props: StageComponentProps) {
  const textProblem = props.problem("S-700-B01");
  const qrProblem = props.problem("S-700-B02");
  const presentationProblem = props.problem("S-700-B03");
  ...
  const [status, setStatus] = useState(() =>
    stageText(props.locale, s700Locale.idle),
  );
  ...
  const connectRemote = async () => {
    ...
    if (!video || !remote?.prompt) {
      setStatus(stageText(props.locale, s700Locale.remoteUnavailable));
      return;
    }
    ...
    try {
      ...
      setStatus(
        stageText(
          props.locale,
          connected ? s700Locale.remoteConnected : s700Locale.remoteWaiting,
        ),
      );
    } catch {
      setStatus(stageText(props.locale, s700Locale.cancelled));
    }
  };
  ...
  const playSegment = async (kind: "text" | "qr") => {
    ...
    if (!video || remote?.state !== "connected") {
      setStatus(stageText(props.locale, s700Locale.remoteRequired));
      return;
    }
    ...
    const reached = () => {
      ...
      if (kind === "text") {
        setTextPlayed(true);
        setStatus(stageText(props.locale, s700Locale.textShown));
      } else {
        setQrPlayed(true);
        setStatus(stageText(props.locale, s700Locale.qrShown));
      }
    };
  ...
  const submitKey = () => {
    ...
    if (
      ...
      setStatus(stageText(props.locale, s700Locale.textSolved));
    } else {
      setStatus(stageText(props.locale, s700Locale.wrongKey));
    }
  };
  ...
  const scanQr = async () => {
    ...
    const video = videoRef.current as
      ...
      | null;
    if (!Detector || !camera) {
      setStatus(stageText(props.locale, s700Locale.barcodeUnavailable));
      return;
    }
    if (!qrPlayed || video?.remote?.state !== "connected") {
      setStatus(stageText(props.locale, s700Locale.qrRequired));
      return;
    }
    ...
    try {
      ...
      while (scanningRef.current && performance.now() < deadline) {
        ...
        if (value === slot.token && video.remote?.state === "connected") {
          ...
          setStatus(stageText(props.locale, s700Locale.qrSolved));
          break;
        }
  ...
  const startPresentation = async () => {
    ...
    if (!Request) {
      setStatus(stageText(props.locale, s700Locale.presentationUnavailable));
      return;
    }
    ...
    try {
      ...
      const onMessage = (event: Event) => {
        ...
        presentationProblem.solve([
          "presentation:receiver-ready",
          `round:${round}`,
        ]);
        setStatus(stageText(props.locale, s700Locale.presentationReady));
      };
      connection.addEventListener("message", onMessage);
      setStatus(stageText(props.locale, s700Locale.presentationWaiting));
    } catch {
      setStatus(stageText(props.locale, s700Locale.cancelled));
    }
  };
  ...
  return (
    <div className="puzzle puzzle--centered s700-stage">
      <div className="problem-row">
        <ProblemGiftBox problem={textProblem} locale={props.locale} />
        <ProblemGiftBox problem={qrProblem} locale={props.locale} />
        <ProblemGiftBox problem={presentationProblem} locale={props.locale} />
      </div>
      <video
        ref={videoRef}
      ...
      <div className="s700-panel-grid">
        <section>
          <p className="s700-panel-heading">
            {stageText(props.locale, s700Locale.remoteHeading)}
          </p>
          <button
            type="button"
          ...
          >
            {stageText(props.locale, s700Locale.connectRemote)}
          </button>
          <button
            type="button"
            className="stage-action"
          ...
          >
            {stageText(props.locale, s700Locale.showText)}
          </button>
          <label className="stage-field">
            <span>{stageText(props.locale, s700Locale.keyLabel)}</span>
            <input
              value={key}
          ...
          <button type="button" className="stage-action" onClick={submitKey}>
            {stageText(props.locale, s700Locale.submitKey)}
          </button>
          <button
            type="button"
            className="stage-action"
          ...
          >
            {stageText(props.locale, s700Locale.showQr)}
          </button>
          <button
            type="button"
            className="stage-action"
            onClick={() => void scanQr()}
          >
            {stageText(props.locale, s700Locale.scanQr)}
          </button>
        ...
        <section>
          <p className="s700-panel-heading">
            {stageText(props.locale, s700Locale.presentationHeading)}
          </p>
          <button
            type="button"
            className="stage-action"
          ...
          >
            {stageText(props.locale, s700Locale.startPresentation)}
          </button>
        </section>
      </div>
