...
### YouTube Playlist Cleaner プロジェクトの実装（完了）

YouTube再生リストから見終えた動画を削除する静的ウェブアプリを新規作成しました。

#### 実装内容

1. **プロジェクト構造の初期セットアップ**
   - ディレクトリ構造の作成
   - 基本ファイル（index.html, main.tsx, App.tsx）の作成
   - vite.config.ts にエントリポイント追加
   - リポジトリのindex.htmlにリンク追加

2. **型定義とAPI基盤**
   - YouTube Data API v3 の型定義（Playlist, PlaylistItem, Video など）
   - API呼び出し関数（再生リスト取得、動画削除、評価など）
   - ユーティリティ関数（時間フォーマット、URL生成など）

3. **OAuth 2.0 認証機能**
   - Google Identity Services を使用した認証実装
   - 認証状態の Jotai atoms
   - ログイン/ログアウト UI コンポーネント
...
5. **動画リスト表示機能**
   - VideoCard コンポーネント（サムネイル、タイトル、評価、削除チェックボックス）
   - VideoManager コンポーネント（動画一覧管理）
   - 動画の詳細情報取得（再生時間など）
   - 「YouTubeで確認」ボタン
...
8. **ドキュメント整備**
   - 詳細な README.md の作成
   - セットアップ手順（Google Cloud Console 設定）
   - 使い方の説明
   - 制限事項とトラブルシューティング
...
- OAuth 2.0 Implicit Grant Flow による認証
- YouTube Data API v3 の活用
- React + MUI + Jotai による状態管理
- レスポンシブデザイン

#### 制限事項

- YouTube Data API では視聴履歴・再生進行状況が取得できない
  - 「YouTubeで確認」ボタンで別タブを開いて確認する仕組みで対応
- ユーザー自身が視覚的に判断して削除する設計
